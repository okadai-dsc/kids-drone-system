...
# --- 離着陸ホームポジション（ケージ座標 m）---------------------------
# 会場配置図（中会議室 D-③）に対応。観客席側の辺を y=0 とし、4機を x 方向一列に置く。
# 機体番号は観客席から見て右（x が大きい側）から順に割り当てる（#1=右端 … #4=左端）。
# tellomon が dead-reckoning の起点に使い、ドローンモニタがマップの目印に使う。
HOME_POSITIONS = {
    1: (5.5, 1.1),  # 右端（観客席から見て）
    2: (4.0, 1.1),
    3: (2.5, 1.1),
    4: (1.0, 1.1),  # 左端
}
HOME_YAW = 0  # 離陸時の機首方向（0 = +Y = 観客席と反対の奥向き）
