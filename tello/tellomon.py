...
def run_udp_receiver():
    ...
    # 外部の転送先のアドレスとポートの定義
    serv2_address = ('192.168.0.100', 11112)
    ...
    while True:
        ...
        try:
            # モニタにビデオを転送する
            send_len = send_sock1.sendto(message, serv1_address)
            # 外部にビデオを転送する
            if VIDEO_FLAG == 1 :
                send_sock2 = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
                send_len = send_sock2.sendto(message, serv2_address)
...
class ComHandler():
    ...
    def send_cmd(self, message):
        ...
        try:
            send_len = self.sock.sendto(message.encode('utf-8'), serv_address)
            ...
            if ( rx_meesage.decode(encoding='utf-8') != "ok" ):
                ...
                for i in range(3):
                    ...
                    rx_message = ""
                    rx_meesage, addr = self.sock.recvfrom(MSG_SIZE)
                    print("send_cmd_retry#Receive Response,Message:" + rx_meesage.decode(encoding='utf-8'))
                    if ( rx_meesage.decode(encoding='utf-8') == "ok" ):
                        break
            ...
            print ("send_cmd#Caught exception socket.error : %s" % exc)
            # レスポンス情報を返却(timeout)            
            return exc
...
#            print("\nreceve_forever#Recv,data:%s " % data.decode(encoding='utf-8'))
            ...
            for val in data_list:
                if( val.split(':')[0] == "h" ):
                    self.view.koudo_var.set(int(val.split(':')[1]))
                    
                if( val.split(':')[0] == "bat" ):
                    self.view.dengen_var.set(int(val.split(':')[1]))
                    
                if( val.split(':')[0] == "time" ):
#                    print("\nreceve_forever#Recv,bat:%s " % val.split(':')[1])
                    self.view.hikouzikan_var.set(int(val.split(':')[1]))
...
#            self.wfile.write(bytes("</body></html>", "utf-8"))
            ...
            # メインスレッドにキューを使用して依頼
            msg_queue.put("%s" % self.path)
            err_queue.put("%s:%s" % (self.path ,trn_msg))
            ...
            self.send_header("Access-Control-Allow-Origin", "*")
            self.end_headers()
            self.wfile.write(bytes("%s" % (trn_msg), "utf-8"))
...
class View():
    def __init__(self, app):
        ...
        self.gif_file.set("./tellomon.gif")
        self.gif_image = tk.PhotoImage(file=self.gif_file.get())
        # ドローンの図形の定義
        self.drone_file = "./drone.gif"
    ...
    def create_widgets(self):
        ...
        self.koudo = tk.Scale(
            self.state_frame11,
            from_=300, to=0, tickinterval=50,
            orient=VERTICAL,
        ...
        self.sokudo = tk.Scale(
            self.state_frame21,
            from_=100, to=0, tickinterval=20,
            orient=VERTICAL,
        ...
        self.hikouzikan = tk.Scale(
            self.state_frame22,
            from_=600, to=0, tickinterval=20,
            orient=VERTICAL,
...
#            width=20,
        ...
        self.Entry1 = tk.Entry(
            self.control_frame21,
            width=10,
            textvariable=self.drone_x_pos
        )   
        self.Entry1.pack()
        
        self.label2 = tk.Label(
            self.control_frame31,
            anchor=NW,
            text="初期位置(Y):"
        )
        self.label2.pack()
        ...
        self.Entry2 = tk.Entry(
            self.control_frame41,
            width=10,
            textvariable=self.drone_y_pos
        )   
        self.Entry2.pack()
        
        self.label3 = tk.Label(
            self.control_frame12,
            anchor=NW,
            text="初期方向:(度)"
        )
        self.label3.pack()
        ...
        self.Entry3 = tk.Entry(
            self.control_frame22,
            width=10,
            textvariable=self.drone_direction
        )   
        self.Entry3.pack()
        
        self.label4 = tk.Label(
            self.control_frame13,
            anchor=NW,
            text="飛行図面:"
        )
        self.label4.pack()
        ...
        self.Entry4 = tk.Entry(
            self.control_frame23,
            width=10,
            textvariable=self.gif_file
        )   
        self.Entry4.pack()
        
        # リセットボタンの作成と配置
        self.reset_button = tk.Button(
            self.control_frame42,
            bg = 'cyan',
            text="リセット"
        )
        ...
        # メッセージ更新用
        self.message = tk.StringVar()
        self.start_button2 = tk.Checkbutton(
            self.msg_frame112,
            selectcolor = 'red',
            text="通信状態"
        )
...
#        self.main_canvas.create_line(350 - DRONE_CAGE_P ,20 + DRONE_CAGE_P ,50 + DRONE_CAGE_P ,20 + DRONE_CAGE_P,  fill='red', width=2, dash=(3,4))
        ...
        for i in range(7):
            ...
            self.main_canvas.create_text(x,324, text='%d'% ((-15 + 5*i) - 0), anchor=N)
        for i in range(7):
            y = 320 - (i * 50)
            self.main_canvas.create_line(50,y,55,y, width=2)
            self.main_canvas.create_text(46,y, text='%d'% ((-10 + 5*i) - 0), anchor=E)
 
        self.draw_drone()
 
    def draw_drone(self):
        # ドローンの開始位置と方向を初期化する
        self.x_addr = CANVAS_X_BASE + ( int(self.drone_x_pos.get()) + 750 )/DRONE_PIXEL + DRONE_X_BASE
        self.y_addr = CANVAS_Y_BASE - ( int(self.drone_y_pos.get()) + 500 )/DRONE_PIXEL - DRONE_Y_BASE
        self.direction = 360 - int(self.drone_direction.get())
...
#    com = ComHandler()
    ...
    t3 = threading.Thread(target=run_udp_receiver, args=())
    t3.daemon = True
    t3.start()
